...
class AddFolderDialog(Gtk.Dialog):
    def __init__(self, folder_owner=None, main_window_instance=None, modify=False):
        super().__init__(title="Add Folder")
        ...
        self.entry_folder_name = Gtk.Entry()
        self.entry_folder_name.set_hexpand(True)
        ...
        if modify:
            ...
            button_save.connect('clicked', self.store_new_folder)

        name_box.add(request_name_label)
        name_box.add(self.entry_folder_name)
